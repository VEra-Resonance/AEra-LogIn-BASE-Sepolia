 ...
 * Loads on-chain data: Identity NFT, Resonance Score, Interaction History
 */
        ...
        try {
            ...
            if (data.has_identity && data.token_id) {
                ...
                updateElement('identityStatus', `
                    <span style="color: #4caf50;">✅ Minted</span>
                    <br>
                ...
                updateElement('identityStatus', `
                    <span style="color: #f44336;">❌ Mint Failed</span>
                    <br>
                    <small>Contact support</small>
                `);
            } else {
                updateElement('identityStatus', `
                    <span style="color: #9e9e9e;">⏳ Not Minted</span>
                    <br>
    ...
    async function loadResonanceScore(address) {
        ...
        try {
            ...
            updateElement('blockchainScore', `
                <span style="color: #4caf50;">✅ ${chainScore}</span>
                <br>
                <small style="font-size: 11px; opacity: 0.85;">
            ...
            updateElement('syncStatus', `
                <span style="color: ${syncColor};">${syncIcon} ${syncText}</span>
                <br>
                <small style="font-size: 10px;">${lastSync}</small>
            `);
            ...
            updateElement('syncStatus', `
                <span style="color: #f44336;">❌ Error</span>
                <br>
                <small style="font-size: 10px;">Check console</small>
            `);
        }
    }
    ...
    async function loadInteractionHistory(address) {
        ...
        try {
            ...
            data.interactions.forEach((interaction, index) => {
                ...
                html += `<td style="padding: 8px;">${typeIcon} ${typeName}</td>`;
        ...
        try {
            ...
            if (health.status === 'connected') {
                statusEl.innerHTML = `
                    ...
                    <br>
                    <small style="font-size: 10px;">Block ${health.block_number || 'N/A'}</small>
                `;
            } else {
                statusEl.innerHTML = `
                    <span style="color: #f44336;">🔴 Offline</span>
                    <br>
                    <small style="font-size: 10px;">${health.error || 'Connection error'}</small>
                `;
            }
